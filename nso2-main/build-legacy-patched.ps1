param(
    [string]$OutputJar = "build\e72_auto_nvc.jar",
...
$runtimeClassesDir = Join-Path $projectDir "latest_classes"
...
$outputPath = Join-Path $projectDir $OutputJar
...
if (!(Test-Path -LiteralPath $jarTool)) {
    throw "Khong tim thay jar.exe trong JDK."
}

if (!(Test-Path -LiteralPath $baseJar) -or !(Test-Path -LiteralPath $microJar)) {
    throw "Thieu e72_x1.jar hoac MICRO.jar."
}
if (!(Test-Path -LiteralPath $manifestPath)) {
    throw "Thieu manifest: $manifestPath"
}
...
$bootstrapClassPath = $microJar + ";" + $runtimeClassesDir + ";" + $baseJar
...
# MICRO.jar khong co JSR-120. Giu SmsData.class tu JAR goc va compile cac source con lai.
...
$classPath = $microJar + ";" + $runtimeClassesDir + ";" + $baseJar + ";" + $classesDir
...
if (Test-Path -LiteralPath $dailyCoordinatorSource) {
    $runtimeClassPath = $microJar + ";" + $runtimeClassesDir + ";" + $baseJar + ";" + $classesDir
    & $javac --release 8 -encoding UTF-8 -cp $runtimeClassPath -d $classesDir $dailyCoordinatorSource
    if ($LASTEXITCODE -ne 0) {
        throw "Compile AutoDailyCoordinator that bai."
    }
}
...
Get-ChildItem $runtimeClassesDir -Filter "*.class" |
...
# QLTK only enables Start for recognised NST-GameID values.  Keep the
# original GameMidlet entry, but expose the AUTO50 identity used by V20/V29.
& $jarTool ufm $outputPath $manifestPath
if ($LASTEXITCODE -ne 0) {
    throw "Cap nhat manifest JAR that bai."
}
Push-Location $classesDir
try {
    & $jarTool uf $outputPath .
    if ($LASTEXITCODE -ne 0) {
        throw "Dong goi JAR that bai."
    }
} finally {
    Pop-Location
}

Write-Host "Build thanh cong: $outputPath"
