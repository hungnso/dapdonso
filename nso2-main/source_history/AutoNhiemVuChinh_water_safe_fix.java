...
extends Auto {
    ...
    private static final long COMBAT_STALL_DELAY = 30000L;
    private static final long COMBAT_ATTEMPT_ACTIVE_DELAY = 3500L;
    private static final int COMBAT_STALL_MIN_ATTEMPTS = 8;
    private static final int JAIAN_DIRECTION_SWITCH_DISTANCE = 24;
    private static final int JAIAN_MOVE_TARGET_TOLERANCE = 20;
    private static final long JAIAN_TERRAIN_STUCK_DELAY = 700L;
    private static final long JAIAN_TERRAIN_MOVE_COOLDOWN = 900L;
    private static final int JAIAN_TERRAIN_VERTICAL_THRESHOLD = 28;
    ...
    private void doTaskThreeCombat(int n, int n2, int n3, int n4) {
        ...
        int n5 = n == 2 ? 1 : 2;
        System.out.println("AutoNVC task3=attack index=" + n + " mobTemplate=" + n5 + " count=" + n4 + "/" + n3 + " map=" + n2);
    ...
    private void doJaianEscort(Char char_, int n) {
        boolean bl;
        int n2;
        Char char_2 = AutoNhiemVuChinh.findJaianEscort(char_);
        if (char_2 != null && TileMap.mapID == 33) {
            this.setJaianFreeze(true);
            this.freezeJaianEscortMobs();
        }
        if (!this.jaianHpStockReady && this.maintainJaianHpStock(char_, char_2 != null)) {
            return;
        }
        if (TileMap.mapID != 33) {
        ...
        long l = System.currentTimeMillis();
        if ((long)char_.cHp * 100L < (long)char_.cMaxHp * 70L && l - this.lastEscortPotionAction >= 800L && !GameCanvas.menu.showMenu && ChatPopup.b == null && GameCanvas.currentDialog == null) {
            GameScr.x();
            this.lastEscortPotionAction = l;
        }
        if (char_2 == null) {
            this.escortSeen = false;
        ...
        if (this.lastEscortX < 0) {
            this.lastEscortX = char_2.cx;
        } else {
            n2 = char_2.cx - this.lastEscortX;
            if (n2 >= JAIAN_DIRECTION_SWITCH_DISTANCE) {
                if (this.escortDirection != 1) {
                    this.escortDirection = 1;
                    char_.currentMovePoint = null;
                    System.out.println("AutoNVC escort=direction-stable direction=1 jaian=" + char_2.cx + "," + char_2.cy);
                }
                this.lastEscortX = char_2.cx;
            } else if (n2 <= -JAIAN_DIRECTION_SWITCH_DISTANCE) {
                if (this.escortDirection != -1) {
                    this.escortDirection = -1;
                    char_.currentMovePoint = null;
                    System.out.println("AutoNVC escort=direction-stable direction=-1 jaian=" + char_2.cx + "," + char_2.cy);
        ...
        n2 = Math.abs(char_.cx - char_2.cx);
        int n3 = Math.abs(char_.cy - char_2.cy);
        int n4 = this.escortDirection * (char_.cx - char_2.cx);
        Mob mob = AutoNhiemVuChinh.findJaianThreat(char_2, this.escortDirection);
        if (n4 < -45 || n2 > 235 || n3 > 85) {
            int n5 = char_2.cx + this.escortDirection * 110;
            if (n5 < 12) {
                n5 = 12;
            }
            this.moveJaianSmooth(char_, n5, char_2.cy);
            if (l - this.lastEscortMove >= 450L) {
                System.out.println("AutoNVC escort=recover-distance me=" + char_.cx + "," + char_.cy + " jaian=" + char_2.cx + "," + char_2.cy + " target=" + n5 + "," + char_2.cy + " distance=" + n2 + "," + n3);
                this.lastEscortMove = l;
            }
            return;
        }
        if (mob != null) {
            Skill skill = char_.myskill;
            int n6 = skill == null || skill.dx < 25 ? 25 : skill.dx;
            int n7 = skill == null || skill.dy < 20 ? 20 : skill.dy;
            int n8 = Math.abs(char_.cx - mob.xFirst);
            int n9 = Math.abs(char_.cy - mob.yFirst);
            if (n8 > n6 || n9 > n7) {
                int n10 = mob.xFirst;
                int n11 = char_2.cx - 220;
                int n12 = char_2.cx + 220;
                if (n10 < n11) {
                    n10 = n11;
                }
                if (n10 > n12) {
                ...
                int n13 = mob.yFirst;
                int n14 = char_2.cy - 55;
                int n15 = char_2.cy + 55;
                if (n13 < n14) {
                    n13 = n14;
                }
                if (n13 > n15) {
                ...
                this.moveJaianSmooth(char_, n10, n13);
                return;
            }
            char_.mobFocus = mob;
            long l2 = System.currentTimeMillis();
            if (l2 - this.lastEscortAttack >= 1200L) {
                System.out.println("AutoNVC escort=protect-jaian mob=" + mob.mobId + " template=" + mob.templateId + " rank=" + mob.levelBoss + " mobAt=" + mob.xFirst + "," + mob.yFirst + " jaianAt=" + char_2.cx + "," + char_2.cy);
                this.lastEscortAttack = l2;
            }
            this.attackTracked(-1, 3);
            return;
        }
        if (n4 >= 90 && n4 <= 145 && n3 <= 55) {
            char_.currentMovePoint = null;
            if (l - this.lastEscortMove >= 3000L) {
                System.out.println("AutoNVC escort=lead-jaian map=" + TileMap.mapID + " me=" + char_.cx + "," + char_.cy + " jaian=" + char_2.cx + "," + char_2.cy + " hp=" + char_2.cHp + " direction=" + this.escortDirection + " lead=" + n4);
                this.lastEscortMove = l;
            }
            return;
        }
        ...
        int n16 = char_2.cx + this.escortDirection * 115;
        if (n16 < 12) {
            n16 = 12;
        }
        bl = this.moveJaianSmooth(char_, n16, char_2.cy);
        this.lastEscortMove = l;
        System.out.println("AutoNVC escort=move-ahead map=" + TileMap.mapID + " me=" + char_.cx + "," + char_.cy + " jaian=" + char_2.cx + "," + char_2.cy + " target=" + n16 + "," + char_2.cy + " distance=" + n2 + "," + n3 + " direction=" + this.escortDirection + " smooth=" + bl);
    }

    private boolean moveJaianSmooth(Char char_, int n, int n2) {
        if (char_ == null) {
            return false;
        }
        long now = System.currentTimeMillis();
        boolean targetChanged = this.jaianMoveTargetX < 0
                || Math.abs(this.jaianMoveTargetX - n) > JAIAN_MOVE_TARGET_TOLERANCE
                || Math.abs(this.jaianMoveTargetY - n2) > 55;
        if (targetChanged) {
            this.jaianMoveTargetX = n;
            this.jaianMoveTargetY = n2;
        }
        ...
        int distanceX = Math.abs(char_.cx - n);
        int distanceY = Math.abs(char_.cy - n2);
        ...
        // Khong teleport theo truc Y khi dang cuu Jaian. Dac biet luc nhan vat
        // dang o nuoc (status 10/11), Char.c() co the gui thang toa do tu nuoc
        // len bo va server se xem day la buoc di khong hop le. Thay vao do, tao
        // mot cu nhay vat ly giong phim len + trai/phai cua game.
        boolean needsUpwardTerrainMove = n2 < char_.cy - JAIAN_TERRAIN_VERTICAL_THRESHOLD;
        boolean stuckOnTerrain = distanceX > 35
                && this.jaianMoveProgressAt > 0L
                && now - this.jaianMoveProgressAt >= JAIAN_TERRAIN_STUCK_DELAY;
        if ((needsUpwardTerrainMove || stuckOnTerrain)
                && now - this.lastJaianTerrainMove >= JAIAN_TERRAIN_MOVE_COOLDOWN
                && this.tryJaianPhysicalJump(char_, n, n2, now)) {
            return true;
        }

        // Dang nhay/roi thi de physics cua game tu xu ly cho den khi cham dat.
        // Khong chen currentMovePoint hay Char.c() vao giua cu nhay.
        if (char_.statusMe == 3 || char_.statusMe == 4) {
            char_.currentMovePoint = null;
            return false;
        }

        // Fallback Char.c chi duoc phep khi bi ket tren cung mat phang. Tuyet doi
        // khong dung no de nhay tu nuoc/duoi thap len toa do Y cao hon.
        boolean sameLevelSafeRoute = stuckOnTerrain
                && distanceY <= 20
                && (char_.statusMe == 1 || char_.statusMe == 2)
                && now - this.lastJaianTerrainMove >= JAIAN_TERRAIN_MOVE_COOLDOWN;
        if (sameLevelSafeRoute) {
            char_.currentMovePoint = null;
            boolean routed = Char.c(n, char_.cy);
            this.lastJaianTerrainMove = now;
            if (routed) {
                this.jaianMoveProgressX = char_.cx;
                this.jaianMoveProgressY = char_.cy;
                this.jaianMoveProgressAt = now;
                System.out.println("AutoNVC escort=terrain-flat-step target=" + n + "," + char_.cy);
                return true;
            }
        }

        MovePoint movePoint = char_.currentMovePoint;
        if (movePoint != null && Math.abs(movePoint.xEnd - n) <= JAIAN_MOVE_TARGET_TOLERANCE
                && Math.abs(movePoint.yEnd - n2) <= 55) {
            return false;
        }
        ...
        char_.currentMovePoint = new MovePoint(n, char_.cy);
        return true;
    }
    ...
    private boolean tryJaianPhysicalJump(Char char_, int targetX, int targetY, long now) {
        ...
        int direction = deltaX < 0 ? -1 : deltaX > 0 ? 1 : char_.cdir;
        if (direction == 0) {
            direction = 1;
        }
        ...
        System.out.println("AutoNVC escort=terrain-jump fromWater=" + fromWater
                ...
                + " speedX=" + horizontalSpeed + " speedY=" + char_.cvy);
        return true;
    }
    ...
    private static Mob findJaianThreat(Char char_, int n) {
        ...
        for (int i = 0; i < GameScr.vMobAttack.size(); ++i) {
            int n4;
            Mob mob3 = (Mob)GameScr.vMobAttack.elementAt(i);
            if (mob3 == null || mob3.hp <= 0 || mob3.status == 0 || mob3.status == 1 || mob3.isBoss || mob3.levelBoss >= 2) continue;
            int n5 = Math.abs(char_.cx - mob3.xFirst);
            int n6 = Math.abs(char_.cy - mob3.yFirst);
            int n7 = n5 + n6;
            if (n5 <= 85 && n6 <= 75 && n7 < n2) {
                mob = mob3;
                n2 = n7;
            }
            if ((n4 = n * (mob3.xFirst - char_.cx)) < -20 || n4 > 260 || n6 > 110 || n7 >= n3) continue;
            mob2 = mob3;
            n3 = n7;
        }
        return mob != null ? mob : mob2;
    }
    ...
    private boolean monitorCombatStall(Char char_) {
        ...
        if (this.combatLastAttempt <= 0L
                || now - this.combatLastAttempt > COMBAT_ATTEMPT_ACTIVE_DELAY
                || this.combatAttempts < COMBAT_STALL_MIN_ATTEMPTS
                || now - this.combatLastProgress < COMBAT_STALL_DELAY) {
            return false;
        }
        this.combatReloginRequested = true;
        System.out.println("AutoNVC combat=force-relogin map=" + TileMap.mapID
                + " mob=" + this.combatMobId + " hp=" + this.combatMobHp
                + " attempts=" + this.combatAttempts
                + " noDamageOrExpMs=" + (now - this.combatLastProgress));
        GameScr.addChatPopup((String)"Danh quai khong mat mau/khong tang EXP, dang dang nhap lai");
