public final class AutoDanhVong extends Auto {
   ...
   private static final long UNKNOWN_QUEST_CANCEL_DELAY = 2200L;
   private static final long SKIP_QUEST_CANCEL_DELAY = 2200L;
   private static final int MAX_UNAVAILABLE_QUEST_SKIPS = 10;
   ...
   private int amejiNpcTemplateId;
   private boolean resetUnknownQuest;
   private boolean cancelUnknownSent;
   private long cancelUnknownAt;
   private boolean skipCurrentQuest;
   private boolean skipCancelSent;
   private long skipCancelAt;
   private String skipReason;
   private String unavailableItemName;
   private int unavailableQuestSkips;
   ...
   public final void g() {
      ...
      this.amejiNpcTemplateId = -1;
      this.resetUnknownQuest = false;
      this.cancelUnknownSent = false;
      this.cancelUnknownAt = 0L;
      this.skipCurrentQuest = false;
      this.skipCancelSent = false;
      this.skipCancelAt = 0L;
      this.skipReason = null;
      this.unavailableItemName = null;
      this.unavailableQuestSkips = 0;
   ...
   public final void update() {
      ...
      if (this.skipCurrentQuest) {
         this.handleSkipCurrentQuest(me, now);
         return;
      }
   ...
   private void acquireQuest(Char me, long now) {
      if (this.resetUnknownQuest || this.existingQuestUnknown) {
         this.handleUnknownExistingQuest(me, now);
         return;
      }
      ...
      this.amejiNpcTemplateId = ameji.template.npcTemplateId;
   ...
   private void handleUnknownExistingQuest(Char me, long now) {
      if (TileMap.mapID != this.schoolMap) {
         this.a(this.schoolMap, -2, -1, -1);
         return;
      }
      Npc ameji = findAmeji();
      if (ameji == null) {
         this.notice("Auto DV: khong thay Ameji de reset NV cu");
         return;
      }
      this.amejiNpcTemplateId = ameji.template.npcTemplateId;

      if (GameCanvas.menu.showMenu) {
         if (!this.cancelUnknownSent) {
            if (GameCanvas.menu.selectCaptionForStandaloneAuto(
                  new String[]{"hủy", "huy"}, "AutoDV cancel-old")) {
               this.cancelUnknownSent = true;
               this.cancelUnknownAt = now;
               this.pendingNpcAction = 4;
               this.lastAction = now;
               this.notice("Auto DV: NV cu khong doc duoc, huy 1 lan de nhan lai");
            }
         }
         return;
      }
      ...
      if (this.cancelUnknownSent) {
         if (now - this.cancelUnknownAt < UNKNOWN_QUEST_CANCEL_DELAY) {
            return;
         }
         // The reputation server does not expose full details of a task that
         // was accepted before this auto was enabled. After one cancellation,
         // receive a fresh task so its popup can be captured deterministically.
         this.resetUnknownQuest = false;
         this.existingQuestUnknown = false;
         this.existingQuestDetected = false;
         this.existingQuestProbeTried = false;
         ...
         this.lastAction = 0L;
         clearCachedQuest();
         this.notice("Auto DV: da reset NV cu, dang nhan NV moi");
         return;
      }
   ...
   private void requestSkipCurrentQuest(String reason, String itemName) {
      if (this.skipCurrentQuest || this.quest == null) {
         return;
      }
      if (this.unavailableQuestSkips >= MAX_UNAVAILABLE_QUEST_SKIPS) {
         this.stoppedUnsupported = true;
         this.notice("Auto DV tam dung: da bo qua " + this.unavailableQuestSkips + " NV khong co do trong shop");
         return;
      }
      this.skipCurrentQuest = true;
      this.skipCancelSent = false;
      this.skipCancelAt = 0L;
      this.skipReason = reason;
      this.unavailableItemName = itemName;
      this.lastAction = 0L;
      Char me = Char.getMyChar();
      if (me != null) {
         me.mobFocus = null;
      ...
      GameCanvas.menu.showMenu = false;
      this.notice("Auto DV: bo NV vi " + reason + ", se Huy va Nhan NV moi");
      System.out.println("AutoDV skip quest reason=" + reason + " item=" + itemName
            + " title=" + this.quest.title + " skipped=" + this.unavailableQuestSkips);
   }

   private void handleSkipCurrentQuest(Char me, long now) {
      if (this.quest == null) {
         this.finishSkippedQuest();
         return;
      }
      if (TileMap.mapID != this.schoolMap) {
         this.a(this.schoolMap, -2, -1, -1);
      ...
      Npc ameji = findAmeji();
      if (ameji == null) {
         this.notice("Auto DV: khong thay Ameji de Huy NV khong co do");
         return;
      }
      this.amejiNpcTemplateId = ameji.template.npcTemplateId;

      if (GameCanvas.menu.showMenu) {
         if (!this.skipCancelSent && GameCanvas.menu.selectCaptionForStandaloneAuto(
               new String[]{"hủy", "huy"}, "AutoDV skip-unavailable")) {
            this.skipCancelSent = true;
            this.skipCancelAt = now;
            this.pendingNpcAction = 5;
            this.lastAction = now;
            this.notice("Auto DV: dang Huy NV - khong co " + this.unavailableItemName + " trong shop");
         }
         return;
      }

      if (this.skipCancelSent) {
         if (now - this.skipCancelAt < SKIP_QUEST_CANCEL_DELAY) {
            return;
         }
         // Some servers do not send a dedicated cancellation acknowledgement.
         // After one Huy command, clear the local task and immediately request a fresh one.
         this.finishSkippedQuest();
         return;
      }
   ...
   private void finishSkippedQuest() {
      ++this.unavailableQuestSkips;
      String oldItem = this.unavailableItemName;
      this.quest = null;
      this.localKills = 0;
      this.localYen = 0;
      this.localUpgradeCount = 0;
      ...
      this.resetUpgrade();
      clearCachedQuest();
      this.notice("Auto DV: da Huy NV khong co " + oldItem + ", dang Nhan NV moi");
      System.out.println("AutoDV skipped unavailable quest item=" + oldItem
            + " totalSkipped=" + this.unavailableQuestSkips);
   }
   ...
   private boolean searchAndBuyRequiredItem(Char me, String name, long now) {
      ...
      if (this.shopCursor >= shopTypes.length) {
         this.requestSkipCurrentQuest("khong tim thay trang bi trong shop", name);
         return false;
      }
      int type = shopTypes[this.shopCursor];
      // Always refresh the shop once before deciding the requested item does
      // not exist. GameScr shop arrays survive for a long time and can be stale
      // after relog/server changes, which made valid items look unavailable.
      if (this.requestedShopType != type) {
         Service.gI().requestItem(type);
         this.requestedShopType = type;
         this.shopRequestAt = now;
         System.out.println("AutoDV refresh shop=" + type + " item=" + name);
         return false;
      }
      if (now - this.shopRequestAt < 900L) {
         return false;
      }
      Item[] shop = getShopItems(type);
      if (shop == null) {
         if (now - this.shopRequestAt >= 2500L) {
            Service.gI().requestItem(type);
            this.shopRequestAt = now;
            System.out.println("AutoDV retry shop=" + type + " item=" + name);
         }
         return false;
      }
   ...
   public static void onTaskPopup(String title, String content) {
      boolean autoActive = NSOT_MOB.b instanceof AutoDanhVong;
      if (!hasReputationFooter(content) && !(autoActive && looksLikeReputationQuest(content))) {
         return;
      }
      // Some servers send a compact reputation popup without the daily-limit
      // footer (only: quest name + "Su dung <item>" + objective).  While
      // Auto DV is active, accept that compact form too so the quest is not
      // ignored and "Nhan" is not pressed forever.
      saveCachedQuest(title, content);
      System.out.println("AutoDV cache reputation popup title=" + title + " compact=" + !hasReputationFooter(content));
   }

   public static void onAlertPopup(String title, String content) {
      boolean autoActive = NSOT_MOB.b instanceof AutoDanhVong;
      if (!hasReputationFooter(content) && !(autoActive && looksLikeReputationQuest(content))) {
         return;
      }
   ...
   public static void onNpcMessage(int npcTemplateId, String message) {
      ...
      Char me = Char.getMyChar();
      Npc focus = me == null ? null : me.npcFocus;
      boolean focusIsAmeji = focus != null && focus.template != null
            && focus.template.name != null && focus.template.name.toLowerCase().indexOf("ameji") >= 0;
      if (focusIsAmeji) {
         auto.amejiNpcTemplateId = focus.template.npcTemplateId;
      }
      // npcFocus is often cleared immediately after a menu command is sent.
      // Use the Ameji id remembered when openMenu() was called instead.
      if (npcTemplateId != auto.amejiNpcTemplateId && !focusIsAmeji) {
         return;
      }

      String lower = message.toLowerCase();
      boolean asksToFinishQuest = lower.indexOf("hoàn thành nhiệm vụ") >= 0
            || lower.indexOf("hoan thanh nhiem vu") >= 0;
      boolean assignedBefore = asksToFinishQuest
            && (auto.pendingNpcAction == 1
               || lower.indexOf("đã giao") >= 0 || lower.indexOf("da giao") >= 0
               || lower.indexOf("giao trước") >= 0 || lower.indexOf("giao truoc") >= 0
               || lower.indexOf("nhiệm vụ trước") >= 0 || lower.indexOf("nhiem vu truoc") >= 0);
      boolean cancelDone = (lower.indexOf("hủy") >= 0 || lower.indexOf("huy") >= 0)
            && (lower.indexOf("nhiệm vụ") >= 0 || lower.indexOf("nhiem vu") >= 0);

      System.out.println("AutoDV Ameji msg npc=" + npcTemplateId + " pending=" + auto.pendingNpcAction + " msg=" + message);

      if (cancelDone && auto.pendingNpcAction == 5) {
         auto.skipCancelAt = 0L;
         auto.finishSkippedQuest();
         return;
      }

      if (cancelDone && auto.pendingNpcAction == 4) {
         auto.cancelUnknownAt = 0L;
         auto.resetUnknownQuest = false;
         auto.existingQuestUnknown = false;
         auto.existingQuestDetected = false;
         auto.cancelUnknownSent = false;
         auto.pendingNpcAction = 0;
         auto.lastAction = 0L;
         clearCachedQuest();
         auto.notice("Auto DV: da huy NV cu, dang nhan NV moi");
         return;
      }
      ...
      long now = System.currentTimeMillis();
      // If we reached this message, the server has a reputation task but the
      // client has no captured description for it. Do not retry Nhan forever.
      // Cancel exactly once, then Nhan again; the fresh task popup is hooked in
      // GameScr.a(...) and the auto can start immediately.
      if (auto.quest == null && !auto.restoreQuestFromTaskOrders(me) && !auto.restoreCachedQuest()) {
         auto.existingQuestDetected = true;
         auto.existingQuestUnknown = true;
         auto.resetUnknownQuest = true;
         auto.existingQuestProbeTried = false;
         auto.existingQuestProbeAt = now;
         auto.pendingNpcAction = 0;
         auto.lastAction = 0L;
         GameCanvas.menu.showMenu = false;
         auto.notice("Auto DV: da co NV truoc, se Huy 1 lan va Nhan lai");
      }
   }
   ...
   private static boolean isReputationPopup(String content) {
      return hasReputationFooter(content) || looksLikeReputationQuest(content);
   }

   private static boolean hasReputationFooter(String content) {
      if (content == null) {
         return false;
      }
   ...
   private static boolean looksLikeReputationQuest(String content) {
      if (content == null) {
         return false;
      }
      String lower = content.toLowerCase();
      // Reputation tasks on this server always require wearing/using a named
      // equipment item.  That marker plus a known reputation objective is much
      // safer than treating every generic "Nhiem vu" popup as reputation.
      boolean hasRequiredEquipment = lower.indexOf("sử dụng ") >= 0 || lower.indexOf("su dung ") >= 0;
      if (!hasRequiredEquipment) {
         return false;
      }
      return lower.indexOf("tinh anh") >= 0
            || lower.indexOf("yên từ quái") >= 0 || lower.indexOf("yen tu quai") >= 0
            || lower.indexOf("quái vật") >= 0 || lower.indexOf("quai vat") >= 0
            || lower.indexOf("nâng cấp") >= 0 || lower.indexOf("nang cap") >= 0
            || lower.indexOf("lôi đài") >= 0 || lower.indexOf("loi dai") >= 0
            || lower.indexOf("chiến trường") >= 0 || lower.indexOf("chien truong") >= 0
            || lower.indexOf("cừu sát") >= 0 || lower.indexOf("cuu sat") >= 0
            || lower.indexOf("thủ lĩnh") >= 0 || lower.indexOf("thu linh") >= 0;
   }
   ...
   public final String toString() {
      if (this.skipCurrentQuest) {
         return this.skipCancelSent ? "Auto Danh Vong - dang Huy NV khong co do"
               : "Auto Danh Vong - bo NV khong co " + (this.unavailableItemName == null ? "do" : this.unavailableItemName);
      }
      if (this.quest == null) {
         if (this.resetUnknownQuest || this.existingQuestUnknown) {
            return this.cancelUnknownSent ? "Auto Danh Vong - dang huy NV cu" : "Auto Danh Vong - reset NV cu";
         }
         if (this.existingQuestDetected) {
