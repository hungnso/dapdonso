public final class AutoDanhVong extends Auto {
   ...
   public final void g() {
      ...
      this.resetUpgrade();
      GameScr.addChatPopup("Auto Danh Vong: dang tim Ameji");
   ...
   private void acquireQuest(Char me, long now) {
      ...
      if (GameCanvas.menu.showMenu) {
         if (GameCanvas.menu.selectCaptionForStandaloneAuto(
               new String[]{"nhận nhiệm vụ", "nhan nhiem vu", "nhận nv", "nhan nv"}, "AutoDV receive")) {
            this.lastAction = now;
            return;
         }
         if (GameCanvas.menu.selectCaptionForStandaloneAuto(
      ...
      if (Math.abs(me.cx - ameji.cx) > 22 || Math.abs(me.cy - ameji.cy) > 22) {
         Char.b(ameji.cx, ameji.cy);
         this.lastAction = now;
         return;
      }
      me.npcFocus = ameji;
   ...
   private void handleReport(Char me, long now) {
      ...
      if (GameCanvas.menu.showMenu) {
         if (GameCanvas.menu.selectCaptionForStandaloneAuto(
               new String[]{"hoàn thành", "hoan thanh", "trả nhiệm vụ", "tra nhiem vu", "báo cáo", "bao cao"}, "AutoDV complete")) {
            this.reportAt = now;
            this.lastAction = now;
            return;
         }
