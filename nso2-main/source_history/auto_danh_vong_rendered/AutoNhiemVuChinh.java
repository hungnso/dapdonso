...
extends Auto {
    ...
    private boolean monitorCombatStall(Char char_) {
        ...
        boolean damageStalled = now - this.combatLastProgress >= COMBAT_STALL_DELAY;
        if (!receiveStalled && !damageStalled) {
            return false;
        }
        return this.forceCombatReconnect(receiveStalled ? "server-khong-phan-hoi" : "danh-khong-mat-mau");
    }
