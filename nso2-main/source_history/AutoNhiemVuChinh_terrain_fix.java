...
extends Auto {
    ...
    private boolean moveJaianSmooth(Char char_, int n, int n2) {
        ...
        // currentMovePoint cua nhan vat chinh chi tu di ngang. Neu Jaian len/xuong
        // bac dia hinh, hoac nhan vat bi ket theo truc X, dung Char.c mot lan de
        // vuot dia hinh. Cooldown giu cho camera khong bi snap lien tuc nhu truoc.
        boolean needsVerticalTerrainMove = distanceY > JAIAN_TERRAIN_VERTICAL_THRESHOLD;
        boolean stuckOnTerrain = distanceX > 35
                && this.jaianMoveProgressAt > 0L
                && now - this.jaianMoveProgressAt >= JAIAN_TERRAIN_STUCK_DELAY;
        if ((needsVerticalTerrainMove || stuckOnTerrain)
                && now - this.lastJaianTerrainMove >= JAIAN_TERRAIN_MOVE_COOLDOWN) {
            char_.currentMovePoint = null;
            boolean routed = Char.c(n, n2);
            this.lastJaianTerrainMove = now;
            if (routed) {
                this.jaianMoveProgressX = char_.cx;
                this.jaianMoveProgressY = char_.cy;
                this.jaianMoveProgressAt = now;
                System.out.println("AutoNVC escort=terrain-step target=" + n + "," + n2
                        + " vertical=" + needsVerticalTerrainMove + " stuck=" + stuckOnTerrain);
                return true;
            }
        }
